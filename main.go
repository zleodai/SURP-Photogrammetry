...
var defaultVoxelSize float64 = 0.025
...
func main() {
	...
	// objExporter.TriangulateVerticesTester(true)

	faces := greedyMesher.GreedyMesh(voxels, 2)
	greedyMesher.GenerateFaceJson(faces)
	vertices, vertexMatrix, vertexMap, faceMap := objExporter.GetVerticesFromFaces(faces)
	fmt.Printf("\nMeshing %d Verts\n", len(vertices))
	meshFaces := objExporter.GetMeshFacesFromVertices(faces, faceMap, vertices, vertexMatrix, vertexMap)
	runtime.GC()
	fmt.Printf("\nGot %d Faces in Mesh\n", len(meshFaces))

	objExporter.VoxelsToJson(voxels)
	objExporter.PointsToJson(vertices)
	fmt.Printf("\nConverting %d Faces", len(meshFaces))
	objExporter.ToOBJFile("TestCube", vertices, meshFaces)
	runtime.GC()
